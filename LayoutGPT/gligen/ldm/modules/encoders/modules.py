...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def forward(self, text, return_pooler_output=False):
        batch_encoding = self.tokenizer(text, truncation=True, max_length=self.max_length, return_length=True,
                                        return_overflowing_tokens=False, padding="max_length", return_tensors="pt")
        # Use actual module device (e.g. after .to("cpu")) — self.device from config may still be "cuda".
        _dev = next(self.transformer.parameters()).device
        tokens = batch_encoding["input_ids"].to(_dev)
...
class FrozenCLIPTextEmbedder(nn.Module):
    ...
    def forward(self, text):
        ...
        tokens = clip.tokenize(text).to(_dev)
