...
def _main(args):
    ...
    for val_id, val_example in tqdm(val_data.items(), total=len(val_data), desc='gpt3'):
        ...
        while True:
            ...
            if args.gpt_type == 'ollama':
                from ollama import chat
                choices = []
                for _ in range(args.n_iter):
                    res = chat(model=args.ollama_model, messages=prompt_for_gpt3)
            ...
            try:
                ...
                prompt_str = ""
                if args.gpt_type == 'gpt3.5':
                    prompt_str = prompt_for_gpt3
                else:
                    for msg in prompt_for_gpt3:
                ...
                for _ in range(args.n_iter):
                    res = model.generate_content(
                        ...
                        generation_config=genai.GenerationConfig(
                            ...
                            max_output_tokens=1024 if args.room=='livingroom' else 512,
                            stop_sequences=["Condition:"],
                            top_p=1.0,
                        )
                    )
